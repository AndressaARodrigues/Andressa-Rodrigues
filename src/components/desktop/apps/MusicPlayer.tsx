...
export function MusicPlayer() {
  const { track, index, playing, progress, toggle, next, prev, select, artworks, pause } =
    useMusic();
  const { t } = useI18n();
  ...
  return (
    <div className="w-full h-full flex flex-col bg-linear-to-b from-neutral-50 to-neutral-100 dark:from-neutral-900 dark:to-neutral-950 text-foreground">
      <div className="p-5 flex gap-4 items-center border-b border-black/10 dark:border-white/10">
        <div
          className={cn(
            "w-24 h-24 rounded-xl bg-linear-to-br shadow-lg relative overflow-hidden",
            track.color,
          )}
        >
          {heroArt && (
            <img src={heroArt} alt="" className="absolute inset-0 w-full h-full object-cover" />
          )}
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-3 mt-2">
            ...
            <button
              onClick={toggle}
              className="p-2 rounded-full bg-pink-400 text-white hover:bg-pink-500"
            >
        ...
        {TRACKS.map((tr, i) => {
          ...
          return (
            ...
            >
              <div
                className={cn(
                  "w-10 h-10 rounded bg-linear-to-br overflow-hidden relative shrink-0",
                  tr.color,
                )}
              >
                {art && (
